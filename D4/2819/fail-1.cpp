...
//	freopen("2819.txt", "r", stdin);
	...
	{
        vector<vector<vector<string>>> digit(7, vector<vector<string>>(16));
        for (int i=0;i<4;i++){
            string a, b, c, d;
            cin >> a >> b >> c >> d;
            ...
            for (int j=0;j<16;j++){     // for each cell
                for (int k=0;k<move[j].size();k++){     // for each adjacent cell
                    for (int l=0;l<digit[i-1][move[j][k]].size();l++){  // for each pre-costructed number
                        string temp = digit[0][j][0] + digit[i-1][move[j][k]][l];
                        vector<string>::iterator it = find(digit[i][j].begin(), digit[i][j].end(), temp);
                        if (it == digit[i][j].end()){
        ...
        for (int j=0;j<16;j++){
            for (int m=0;m<digit[6][j].size();m++){
                vector<string>::iterator it = find(total.begin(), total.end(), digit[6][j][m]);
                if (it == total.end()){
